...
$payload = [
    'from' => (string)($config['CONTACT_FROM_EMAIL'] ?? 'ZENKAI OS Website <onboarding@resend.dev>'),
    'to' => [(string)($config['CONTACT_TO_EMAIL'] ?? 'ai@zenkai-os.jp')],
    'reply_to' => $data['email'],
    'subject' => $subject,
    'html' => $html,
];
...
if ($transport === 'mail') {
    $mailFrom = (string)($config['CONTACT_MAIL_FROM']
        ?? 'ZENKAI OS Website <abc-materia@abc-materia.sakura.ne.jp>');
